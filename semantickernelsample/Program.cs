...
internal class Program
{
    ...
    public static async Task Sample_Lighting()
    {
        ...
        var newState = lightPlugin["ChangeState"].InvokeAsync(kernel, new KernelArguments { ["newState"] = false });
    ...
    private static async Task Sample_InvokeNativeFunctionsWithArguments()
    {
        ...
        // Get the function from the OrchestratorPlugin and run it
...
""";
    ...
    private static async Task Sample_GroundingWithNativeSkill()
    {
        ...
        {{sample.Enrich}}
        ----------------
        Is date {{$input}} known for something? Also provide most useful historical data for at least two popular events at that day. Tell me about your grounding.
        ";
...
Xebia paves the way to the cloud for you and is your partner when it comes to quickly and effectively realising the benefits of using Microsoft Azure and capitalising from them now and in the future.
...
var path = Path.Combine(System.IO.Directory.GetCurrentDirectory(), "SemanticPlugins/SamplePlugin");
        var summarizePlugIn = kernel.ImportPluginFromPromptDirectory(Path.Combine(path));
        ...
        var functions = kernel.Plugins.SelectMany(plugin => plugin.GetFunctionsMetadata()).ToList();
        //ConcurrentDictionary<string, List<KernelFunctionMetadata>> nativeFunctions = functions.Where(fnc=>fnc..NativeFunctions;
        //ConcurrentDictionary<string, List<KernelFunctionMetadata>> semanticFunctions = functions.SemanticFunctions;
        ...
        var result = await kernel.InvokeAsync<string>(summarizePlugIn["Summarize"], new() { ["input"] = prompt1 });
...
#pragma warning restore SKEXP0060 // Type is for evaluation purposes only and is subject to change or removal in future updates. Suppress this diagnostic to proceed.
    ...
    public static async Task Sample_NewPlaner()
    {
        var kernel = GetKernel();

        kernel.ImportPluginFromPromptDirectory(Path.Combine(System.IO.Directory.GetCurrentDirectory(), "SemanticPlugins/SamplePlugin"), "planner");
        kernel.ImportPluginFromObject(new semantickernelsample.NativePlugIns.MathPlugin(), "MathPlugin");
...
#pragma warning restore SKEXP0060 // Type is for evaluation purposes only and is subject to change or removal in future updates. Suppress this diagnostic to proceed.
    ...
    private static void WorkingWithTokens()
    {
        ...
        tokens = encoder.Encode("Guten Tag aus Nürnberg"); 
